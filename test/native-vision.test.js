...
test('actual radar integration follows local selection including deselection and list tails', () => {
  const source = fs.readFileSync(sourceFile('SCDEFogOfWarPlugin.cs'), 'utf8');
  const paint = source.slice(source.indexOf('        private void PaintRadarUnitMarkers('),
    source.indexOf('        internal struct RadarSubmitState'));
  const fixture = `using System;
using System.Collections.Generic;
using SCDEFogOfWar;
static class Mathf {internal static int RoundToInt(float f){return (int)Math.Round(f);}}
class FatControler {internal static FatControler instance=new FatControler();internal int SHRadarRectSize=25;}
static class EngineInterface {internal class PlayState {internal int numSelectedChimps;internal int[] selectedChimps;}}
class GameData {internal static GameData Instance=new GameData();internal EngineInterface.PlayState lastGameState;}
struct NativeVisionUnit {internal int Id,Owner;internal float LogicX,LogicY;}
class Fixture {
 readonly RadarUnitMarkers _radarUnitMarkers=new RadarUnitMarkers();
 readonly HashSet<int> _radarSelectedUnitIds=new HashSet<int>();
 readonly List<NativeVisionUnit> _nativeVisionUnits=new List<NativeVisionUnit>();
 readonly int[] _radarFogIndices=new int[625]; readonly float[] _visible={1};
 RadarMarkerProjection _radarMarkerProjection=new RadarMarkerProjection{SpanX=1,SpanY=1,UX=1,VY=1,Width=1,Height=1};
 int _activePlayer=1; const float VisibleThreshold=.35f;
 bool IsVisionPlayer(int owner){return owner==1||owner==2;}
 bool TryLogicToFogIndex(int x,int y,out int index){index=0;return true;}
 ${paint}
 void CheckPixel(byte[] pixels,int x,byte green,string message) {
  int i=(12*25+x)*4;if(pixels[i]!=255||pixels[i+1]!=green||pixels[i+2]!=(green==255?255:50))throw new Exception(message);
 }
 void Run(){
  _nativeVisionUnits.Add(new NativeVisionUnit{Id=1,Owner=1,LogicX=.25f,LogicY=.5f});
  _nativeVisionUnits.Add(new NativeVisionUnit{Id=2,Owner=1,LogicX=.75f,LogicY=.5f});
  _nativeVisionUnits.Add(new NativeVisionUnit{Id=3,Owner=2,LogicX=.5f,LogicY=.5f});
  var state=new EngineInterface.PlayState{numSelectedChimps=1,selectedChimps=new[]{1,2,3}};
  GameData.Instance.lastGameState=state;var pixels=new byte[2500];PaintRadarUnitMarkers(pixels,25,25);
  CheckPixel(pixels,6,255,"selected unit not white");CheckPixel(pixels,18,200,"stale selection tail used");
  state.selectedChimps=new[]{2,3};state.numSelectedChimps=2;PaintRadarUnitMarkers(pixels,25,25);
  CheckPixel(pixels,6,200,"previous selection not blue");CheckPixel(pixels,18,255,"new selection not white");
  CheckPixel(pixels,12,200,"ally selection mistaken for local selection");
  state.numSelectedChimps=0;PaintRadarUnitMarkers(pixels,25,25);CheckPixel(pixels,18,200,"deselection stale");
  state.numSelectedChimps=20;state.selectedChimps=new[]{1};PaintRadarUnitMarkers(pixels,25,25);
  CheckPixel(pixels,6,255,"selection capacity clamp");
  state.selectedChimps=null;PaintRadarUnitMarkers(pixels,25,25);CheckPixel(pixels,6,200,"null list retains white");
  GameData.Instance.lastGameState=null;PaintRadarUnitMarkers(pixels,25,25);
  if(_radarSelectedUnitIds.Count!=0)throw new Exception("null state retains selected ids");
  Console.WriteLine("RADAR_LOCAL_SELECTION_OK");
 }
 static void Main(){try{new Fixture().Run();}catch(Exception e){Console.Error.WriteLine(e.Message);Environment.ExitCode=1;}}
}`;
  assert.match(run([sourceFile('RadarUnitMarkers.cs')], fixture), /RADAR_LOCAL_SELECTION_OK/);
});

test('authoritative unit ownership and live slots survive allocation cursor recycling', () => {
  const result = run(['NativeUnitVisionReader.cs', 'NativeBuildingVisionReader.cs', 'NativeFogCoordinates.cs'].map(sourceFile)
    .concat(path.join(__dirname, 'NativeUnitOwnerTests.cs')));
  assert.match(result, /NATIVE_UNIT_OWNER_OK/);
...
test('native building reconciliation waits one second between reads', () => {
  const source = fs.readFileSync(sourceFile('SCDEFogOfWarPlugin.cs'), 'utf8');
...
class Fixture {
 ...
 void Run(){
  ...
  foreach(float t in new float[]{0.12f,0.75f,0.99f}){Time.unscaledTime=t;RefreshStructureSources();}
  Check(_nativeUnitVisionReader.calls==1,"native table re-read before one second");
  EngineInterface.FlattenedLandscape=true; Time.unscaledTime=1f; RefreshStructureSources();
  Check(_nativeUnitVisionReader.calls==2&&reconciliations==2&&fallbacks==0,"one-second/flattened scan missing");
  Time.unscaledTime=1.99f; RefreshStructureSources(); Check(_nativeUnitVisionReader.calls==2,"early repeat");
  Time.unscaledTime=2f; RefreshStructureSources(); Check(_nativeUnitVisionReader.calls==3,"second interval");
  _nextStructureScan=0; Time.unscaledTime=2.1f; RefreshStructureSources();
  ...
  Console.WriteLine("BUILDING_ONE_SECOND_CADENCE_OK");
 }
 static void Main(){try{new Fixture().Run();}catch(Exception e){Console.Error.WriteLine(e.Message);Environment.ExitCode=1;}}
}`;
  assert.match(run([], fixture), /BUILDING_ONE_SECOND_CADENCE_OK/);
});
