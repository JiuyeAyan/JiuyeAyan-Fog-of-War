...
internal static class NativeUnitOwnerTests
{
    private static void Main()
    {
        ...
        byte[] bytes = new byte[0x65c + 10000 * 0x490];
        ...
        try
        {
            ...
            // The allocation cursor can move below live slots. It is not the
            // active table length; even the final usable slot must remain visible.
            IntPtr last = IntPtr.Add(manager, 0x65c + 9999 * 0x490);
            Marshal.WriteInt16(last, 0x88, 2);
            Marshal.WriteInt16(last, 0x8a, 22);
            Marshal.WriteInt16(last, 0xc0, 700);
            Marshal.WriteInt16(last, 0xc2, 710);
            Marshal.WriteByte(last, 0x92, 1);
            foreach (int cursor in new[] { 10000, 2, 0, 10 })
            {
                Marshal.WriteInt32(manager, cursor);
                if (!reader.TryReadActive(units) || units.Count != 8 || units[7].Id != 9999)
                    throw new Exception("Live units lost when allocation cursor is " + cursor);
            }
            Marshal.WriteByte(last, 0x92, 8);
            if (!reader.TryReadActive(units) || units[7].Owner != 8) throw new Exception("Reused enemy slot retained friendly owner");
            Marshal.WriteInt16(last, 0x88, 3);
            if (!reader.TryReadActive(units) || units.Count != 7) throw new Exception("Final slot death retained");
            Stopwatch watch = Stopwatch.StartNew();
            for (int run = 0; run < 1000; run++) reader.TryReadActive(units);
            Console.WriteLine("Full unit table average ms=" + watch.Elapsed.TotalMilliseconds / 1000);
